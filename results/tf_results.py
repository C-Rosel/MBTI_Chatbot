...
# S/N
# read labeled data
df = pd.read_json("data/tf_training.jsonl", lines=True)
...
X = embeddings
y = (df["label"] == 'T').astype(int).values # label-based indexer, 1 = T, 0 = F
...
y_prob = TF_model.predict_proba(X_test)[:,1]
...
sns.heatmap(conf_matrix, annot=True, fmt='d', cmap='Oranges', xticklabels=['F', 'T'], yticklabels=['F', 'T'])

plt.xlabel('Predicted')
plt.ylabel('True')
plt.title('T/F Confusion Matrix')
